...
(**
Table of CUDA cores per streaming multiporcessor for different GPU device generations.
*)
let coresPerSM =
    [
...
(**
Function to query and print the device capabilities.
*)
let query (dev:Device) =
    ...
    let cudaCores = cudaCoresPerSM * multiProcessorCount
    printfn "  (%2d) Multiprocessors x (%3d) CUDA Cores/MP:    %d CUDA Cores" multiProcessorCount cudaCoresPerSM cudaCores
...
(**
To get CUDA driver version, we use CUDA Drive API
*)
let mutable driverVersion = 0
cuSafeCall(cuDriverGetVersion(&&driverVersion))
printfn ""
printfn "CUDA Driver Version: %d.%d" (driverVersion/1000) ((driverVersion%100)/10)

(**
Note that only devices of Fermi architecture or new will be in AllDevices
*)
